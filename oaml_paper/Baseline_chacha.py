...
for i in range(X.shape[0]):
    ...
    for zz in range(len(group_indexes)):
        ns_features = ' '.join('{}:{:.6f}'.format(
            ind, X.iloc[i, ind]) for ind in group_indexes[zz])
...
# setup autoVW
autovw_ni = AutoVW(max_live_model_num=5, search_space={
                   'interactions': AutoVW.AUTOMATIC, 'quiet': ''})
