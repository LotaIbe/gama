...
link_to_files = "/Users/lota/Library/CloudStorage/OneDrive-BournemouthUniversity/Bournemouth Uni/Semester 3/Dissertation/gama/oaml_paper/regression_results/"
# read the file into a list of lines and print to separate files for different outputs
for d in [0, 1, 2]:
    for version in ["basic", "ensemble"]:
        with open(link_to_files + "/results_extended_" + version + "_" + str(d) + ".txt", 'r') as f:
            lines = f.read().split("\n")
        output_file_perf = open(link_to_files + version
                                + "/Performance_Output_" + str(d) + ".txt", "w+")
        output_file_retrainings = open(
            link_to_files + version + "/Retrainings_Output_" + str(d) + ".txt", "w+")
        output_file_update = open(link_to_files + version
                                  + "/Update_Output_" + str(d) + ".txt", "w+")
...
for d in [0, 1, 2]:  # data streams
    ...
    for version in ["basic", "ensemble"]:
        ...
        read_perf = pd.read_table(
            link_to_files + version + "/Performance_Output_" + str(d) + ".txt", header=None, sep=' ')
        results["AutOL_Data_" + str(d)][version] = (read_perf.iloc[:, [3, 6]])
        results["AutOL_Data_"
                + str(d)][version].columns = ['Batch number', 'Test score']
        ...
        if os.path.getsize(link_to_files + version + "/Retrainings_Output_" + str(d) + ".txt") > 0:
            read_retraining = pd.read_table(
                link_to_files + version + "/Retrainings_Output_" + str(d) + ".txt", header=None, sep=' ')
            retrainings["AutOL_Data_"
                        + str(d)][version] = read_retraining.iloc[:, [5, 0]]
            retrainings["AutOL_Data_"
                        + str(d)][version].columns = ['Batch number', 'Type']
        ...
        if version != "basic" and os.path.getsize(link_to_files + version + "/Retrainings_Output_" + str(d) + ".txt") > 0:
            read_update = pd.read_table(
                link_to_files + version + "/Update_Output_" + str(d) + ".txt", header=None, sep=' ')
            retrainings["AutOL_Data_"
                        + str(d)][version]['Model update'] = read_update.iloc[:, [6]]
...
for d in [0, 1, 2]:
    ...
    ax.set(xlabel="Batch #", ylabel="RMSE")
